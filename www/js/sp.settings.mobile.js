...
ShiftPlanningSettings.prototype.overviewEvents = function() {
	...
	$('#da_se_ov_aa a').bind(clickEvent, function(e) {
		...
		e.stopPropagation();
		objAdmin = $(this);
		if ($(this).attr('type') == "manualyActivate") {
			sp.settings.adminActions(objAdmin);
		} else {
			var title = "Disable user account";
			if ($(this).attr('type') == "delete") {
				title = "Delete user account";
			} else if ($(this).attr('type') == "activate") {
				title = "Activate user account";
			}
			navigator.notification.confirm(_s('Are you sure?'), confAdmin, title);
		}
	});
	$('#da_se_rs_li .fr').live(clickEvent, function(e) {
		e.preventDefault()
	...
	$('#da_se_us_li .fr').live(clickEvent, function(e) {
		e.preventDefault()
		switch ($('#menu .mainNav .active').attr('id')) {
			case 'menu_staff':
				sp.schedule.fromStaff = true;
	...
	$('#settings .mainSub.settings .subNav a').live(clickEvent, function(e) {
		e.preventDefault();
...
ShiftPlanningSettings.prototype.prefillOverview = function(employee) {
	...
	try{
		...
		$('#da_se_ov_cu').html('<li class="additional noBorder"><p>No custom fields to display</p></li>');
	}
	try{
	...
	sp.api('staff.employee', method, data, function(response) {
		sp.staff.getStaff(function() {
			if (type == 'deactivate') {
				...
				$('.settings .mainSub .backMenu').trigger(clickEvent);

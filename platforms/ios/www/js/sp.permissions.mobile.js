...
ShiftPlanningPermissions.prototype.preparePermissions = function() {
	...
	//console.log( 'group: ' + group + ', this.scheduler: ' + this.scheduler );
	if (group >= this.scheduler) {
		//console.log(' treba da ih skloni');
		$('#menu_reports').remove();
		$('#reports').remove();
...
//        $('#da_se_ov_aa').remove();
	...
	if (group > this.scheduler && parseInt(perms.tc_empl_addtime) == 0) {
	    console.log("SCHEDULER JE VAISTINU");
	...
	if (parseInt(perms.timeclock) == 0) {
		...
		$('#timeClock').remove();
		//$('.widgets>li.timeClock').unbind(clickEvent);
		//$('.widgets>li.timeClock').remove();

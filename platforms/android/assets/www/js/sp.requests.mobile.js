...
ShiftPlanningRequests.prototype.vacationEvents = function() {
	...
	$('#rq_va a.deleteVacation').live(clickEvent, function(e) {
		e.preventDefault();
...
ShiftPlanningRequests.prototype.availableEvents = function() {
	...
	/*open shifts */
	$('#rq_av_pu_li a').live(clickEvent, function(e) {
		e.preventDefault();
	...
	/*shift swap*/
	$('#rq_av_sw_li a').live(clickEvent, clickEvent, function(e) {
		e.preventDefault();
	...
	/*shift trades*/
	$('#rq_av_tr_li a').live(clickEvent, function(e) {
		e.preventDefault();
...
ShiftPlanningRequests.prototype.openShiftsEvents = function() {
	var self = this;
	$('#rq_os_os a').live(clickEvent, function(e) {
		e.preventDefault();
		self.current = self.shifts[$(this).attr('rel')];
		sp.loadSubPage('', 'requests', 'openShiftsOpen');
	});

	$('#rq_os_spr a').live(clickEvent, function(e) {
		e.preventDefault();
		self.current = self.shiftsR[$(this).attr('rel')];
		sp.loadSubPage('', 'requests', 'openShiftsRequest');
	});

	$('#rq_os_os_s #rq_os_rtw.green').live(clickEvent, function(e) {
		e.preventDefault();
		var obj = $(this);
	...
	$('#rq_os_os_s #rq_os_rtw.grey').live(clickEvent, function(e) {
		e.preventDefault();
		var obj = $(this);
		obj.addClass('loading');
	...
	$('#rq_os_spr_s .step a').live(clickEvent, function(e) {
		e.preventDefault();
		var obj = $(this);
		obj.addClass('loading');
		var data = {
...
ShiftPlanningRequests.prototype.shiftTradesEvents = function() {
	...
	$('#rq_st_mst_s .traders a').live(clickEvent, function(e) {
		//console.log("#rq_st_mst_s on click");
		var obj = $(this);
		obj.addClass('loading');
		e.preventDefault();
        ...
        //console.log('update Trade => ' + JSON.stringify(data) );
...
//            console.log("Err at #rq_st_mst_s .traders a => " + eee.getMessage() )
		...
		spModel.schedule.update(tradeType, data, function(response) {
            //console.log('update Trade RESPONSE => ' + JSON.stringify(response) );
			obj.removeClass('loading');
			$('.subNavigation .requests li a[subpage=shiftTrades]').trigger(clickEvent);
		}, function(response) {
	...
	$('#rq_st_mts_sub ul a').bind(clickEvent, function(e) {
		...
		if ($(this).hasClass('activate')) {
			...
			navigator.notification.confirm(_s('Are you sure you want to cancel this request?'), confRequest, "Cancel Request");
		}
	});
...
ShiftPlanningRequests.prototype.shiftApprovalsEvents = function() {
	...
	$('#rq_sa_ho span.names, #rq_sa_ho span.time').live(clickEvent, function() {
...
ShiftPlanningRequests.prototype.prepareShiftApprovals = function(data) {
	...
	$.each(data, function(i, item) {
        ...
        if( item.schedule_name == "" || item.schedule_name == null ){
            item.schedule_name = sp.schedule.getScheduleName(item.schedule);
        }
...
ShiftPlanningRequests.prototype.deductWeekends = function(data) {
